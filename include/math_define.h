...
inline void hat_map(const Eigen::Vector3d &w, Eigen::Matrix3d &skew){
	skew.setZero();
	skew(1,0) = w(2);
	skew(0,1) = -w(2);
	skew(2,0) = -w(1);
...
inline void vee_map(const Eigen::Matrix3d &skew, Eigen::Vector3d &w){
	w(0) = skew(2,1);
	w(1) = skew(0,2);
	w(2) = skew(1,0);
}

inline void skew_mat(const Eigen::VectorXd &X, Eigen::MatrixXd &skewmatrix){
	skewmatrix.setZero();

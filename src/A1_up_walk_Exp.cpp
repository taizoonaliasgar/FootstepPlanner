...
void controller(std::vector<raisim::ArticulatedSystem *> A1, LocoWrapperwalk *loco_obj, SRBNMPC* loco_plan, casadi::Function solver, size_t controlTick, raisim::Contact contactInstance) {
    ...
    // Eigen::VectorXd jointVelTotalJ = Eigen::MatrixXd::Zero(TOTAL_DOF,1);
    // jointVelTotalJ = jointVelTotal; 
    // jointVelTotalJ.segment(3,3) = rotE*jointVelTotal.segment(3,3); // convert to body frame, like robot measurements
    // raisim::Vec<3> FRfoot;
    // A1.back()->getFrameVelocity("FR_foot_fixed",FRfoot);
    // raisim::Vec<3> FLfoot;
    // A1.back()->getFrameVelocity("FL_foot_fixed",FLfoot);
    // raisim::Vec<3> RRfoot;
    // A1.back()->getFrameVelocity("RR_foot_fixed",RRfoot);
    // raisim::Vec<3> RLfoot;
    // A1.back()->getFrameVelocity("RL_foot_fixed",RLfoot);
    // Eigen::Matrix<double, 3, 4> foot_vel = loco_obj->getfootv(jpos,jointVelTotalJ);
    // comparingfootv(FRfoot,FLfoot,RRfoot,RLfoot,foot_vel);
...
// Eigen::Matrix<double,15,1> q_est = loco_obj->getStateEstimate(jpos,jointVelTotal);
        // std::cout << q0(0)-q_est(0) << "\t" << q0(1)-q_est(1) << "\t" << q0(2)-q_est(2) << "\t" << 
        //                     q0(3) << "\t" << q0(4) << "\t" << q0(5) << "\t" <<
        //                     q_est(3) << "\t" << q_est(4) << "\t" << q_est(5) << "\t" << 
        //                     q_est(6) << "\t" << q_est(7) << "\t" << q_est(8) << "\t" <<
        //                     q_est(9) << "\t" << q_est(10) << "\t" << q_est(11) << "\t" <<
        //                     q_est(12) << "\t" << q_est(13) << "\t" << q_est(14) << "\t" << std::endl;

        
        //std::cout << q0(0)-q_est(0) << "\t" << q0(1)-q_est(1) << "\t" << q0(2)-q_est(2) << "\t" << 
        //                q0(3)-q_est(3) << "\t" << q0(4)-q_est(4) << "\t" << q0(5)-q_est(5) << std::endl;
        //std::cout << "q0" << "\t" << q0(0) << "\t" << q0(1) << "\t" << q0(2) << std::endl;
        //std::cout << "q_est" << "\t" << q_est(0) << "\t" << q_est(1) << "\t" << q_est(2) << std::endl;

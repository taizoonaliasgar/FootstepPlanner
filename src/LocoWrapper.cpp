...
void LocoWrapper::calcTau2(const double q[18], const double dq[18], const double R[9], size_t gait, size_t ctrlTick, size_t solveduration){
    ...
    if(gait==STAND){
        ...
        if(readytowalkf){
            ...
            if (HLMTphase == 0 || HLMTphase == 20 || ctrlTick == switchtime*ctrlHz + 18*(shifttime2+movetime3) + shifttime2){ 
...
void LocoWrapper::setfinalCoM2(int settlesteps){
    ...
    Eigen::Matrix<double,4,1> CoM_final = Eigen::MatrixXd::Zero(4,1);
    if(settlesteps>4){
        CoM_final(0) = (kin->toePos(0,2)+kin->toePos(0,3))/2+std::floor((settlesteps-4)/2)*0.05;
    }else{
...
void LocoWrapper::ExpWrapper(const double jpos_est[18], const double jvel_est[18], const double rotMatrixDouble[9], size_t control_Tick, size_t solveduration, 
    ...
    if(control_Tick < loco_start_e+shifttime){
        ...
        if(stepind_e<maxsteps){
            ...
            // if(control_Tick==loco_start_e + shifttime + movetime){tookfirststep();}
            // if(control_Tick==loco_start_e + 2*shifttime + 2*movetime){tooksecondstep();}
            if(stepind_e%2==0){
        ...
        if(control_Tick == switchtime*ctrlHz+18*(shifttime2+movetime3)+shifttime2){startwalking();}//break;}

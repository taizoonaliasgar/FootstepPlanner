    ...
    if(gait==STAND){
        ...
        if(readytowalkf){
            ...
            if (HLMTphase == 0 || HLMTphase == 15 || ctrlTick == switchtime*ctrlHz + 10*(shifttime2+movetime3) + shifttime2){ 
                ...
                phaseVar = getPhase(1.0*locoTick, 0.0, 299);
                flphase = getPhase(1.0*locoTick-79, 0.0, 215);
                rlphase = getPhase(1.0*locoTick-119, 0.0, 175);
                flphase = (flphase<0)?0:(flphase>1)?1:flphase;
                rlphase = (rlphase<0)?0:(rlphase>1)?1:rlphase;
                // if(locoTick==50||locoTick==80){
...
void LocoWrapper::ExpWrapper(const double jpos_est[18], const double jvel_est[18], const double rotMatrixDouble[9], size_t control_Tick, size_t solveduration, 
    ...
    if(control_Tick < loco_start_e+shifttime){
        ...
        if(control_Tick == (switchtime+17)*ctrlHz){LL->MPCWalking();readytoreallywalk();}
        // if(control_Tick == (switchtime+17)*ctrlHz){readytoreallywalk();}//break;}
        // if(control_Tick == (switchtime+23)*ctrlHz){readytoreallywalk();}//break;}

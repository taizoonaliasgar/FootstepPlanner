...
SRBNMPC::SRBNMPC(int argc, char *argv[], int numRobots, int id) : Parameters(argc,argv){
    ...
    contact_sequence_dm30(1,casadi::Slice(38,60)) = casadi::DM::zeros(1,22);
    contact_sequence_dm30(2,casadi::Slice(42,60)) = casadi::DM::zeros(1,18);
    contact_sequence_dm30(0,casadi::Slice(8,30)) = casadi::DM::zeros(1,22);
    contact_sequence_dm30(3,casadi::Slice(12,30)) = casadi::DM::zeros(1,18);

    contact_sequence_dm15(1,casadi::Slice(19,30)) = casadi::DM::zeros(1,11);
    contact_sequence_dm15(2,casadi::Slice(21,30)) = casadi::DM::zeros(1,9);
    contact_sequence_dm15(0,casadi::Slice(4,15)) = casadi::DM::zeros(1,11);
    contact_sequence_dm15(3,casadi::Slice(6,15)) = casadi::DM::zeros(1,9);
...
void SRBNMPC::generator(){
    ...
    // file name
    std::string file_name = "Go2_w0p2_ro_0p05_33b";//"take2_1";
    // code predix
...
casadi::SX SRBNMPC::UpdateCostN(casadi::SX x, casadi::SX x_des){
    ...
    Q.block(3,3,3,3).diagonal() <<  5e2,5e2,1e3;//4e5//5e4//mpc_params.qvx, mpc_params.qvy, mpc_params.qvz;
    ...
    Q.block(12,12,4,4).diagonal() <<  1e3,1e3,1e3,1e3;//1e-2//1e3//1e5,1e5,1e5,1e5;
...
casadi::DM SRBNMPC::motionPlannerN2(Eigen::Matrix<double,16,1> q0, size_t controlTick){
    ...
    if((controlTick)%20 == 0){
        x_dom_init = q0_dm(0);
        Raibstep = 0*2*(Tstance*localvelocity+sqrt(stand_height/9.81)*(q0(3)-localvelocity));//0.5*Tstance*(q0(3));//+ sqrt(9.81/stand_height)*(q0(3)-localvelocity);
        //absRaibstep = abs(Raibstep);//0.5*Tstance*abs(q0(3));
    }
    ...
    if(controlTick%20<9){
        ...
        vRaibstep = 0*2*(Tstance*localvelocity+sqrt(stand_height/9.81)*(q0(3)-localvelocity));
    }
    casadi::DM x0 = x_dom_init;
    ...
    for(size_t i= 0; i< HORIZ; i++){
        ...
        //if(controlTick+i>0){
            conp1_next = 0;//(controlTick+i+1)%40;
        //}
        if((controlTick+i)%20 == 0){
            x0 = x_des(i*NFS);
        }
...
int* SRBNMPC::returnConInd30(size_t controlTick){
    ...
    conInd[0] = static_cast<double>(contact_sequence_dm30(0,conmark));
    conInd[1] = static_cast<double>(contact_sequence_dm30(1,conmark));
    conInd[2] = static_cast<double>(contact_sequence_dm30(2,conmark));
    conInd[3] = static_cast<double>(contact_sequence_dm30(3,conmark));
    conInd[4] = conmark;
    return conInd;
}
...
void SRBNMPC::getprevioussol_fullsimMT(casadi::DM q0, Eigen::Matrix<double,3,4> foothold, Eigen::Matrix<double,12,1> forceQP, size_t controlTick){
        

    x0_MT(casadi::Slice(0,12)) = q0(casadi::Slice(0,12));
    x0_MT(12) = foothold(0,0);//0.02
    x0_MT(13) = foothold(0,1);
...
casadi::DM SRBNMPC::motionPlannerN_MT(casadi::DM q0, size_t controlTick){
...
casadi::DM SRBNMPC::motionPlannerN_MT30(casadi::DM q0, size_t controlTick){
    ...
    //     x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+3) = 1;//contact_sequence_dm(3,conp1);
    // }else{
        conm1 = (controlTick-1)%60;
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)) = contact_sequence_dm30(0,conm1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+1) = contact_sequence_dm30(1,conm1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+2) = contact_sequence_dm30(2,conm1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+3) = contact_sequence_dm30(3,conm1);
    // }
    ...
    for(size_t i= 0; i< HORIZ; i++){
        ...
        //if(controlTick+i>0){
            conp1_next = (controlTick+i+1)%60;
        //}
        if((controlTick+i)%30 == 0){
            x0 = x_des(i*NFS);
        }

        x_des((i+1)*NFS+12) = contact_sequence_dm30(0,conp1_next)*contact_sequence_dm30(0,conp1)*x_des(i*NFS+12)+
                                (1-contact_sequence_dm30(0,conp1)*contact_sequence_dm30(0,conp1_next))*(x0+front_off+3/2*Tstance*localvelocity);
                                   
        x_des((i+1)*NFS+13) = contact_sequence_dm30(1,conp1_next)*contact_sequence_dm30(1,conp1)*x_des(i*NFS+13)+
                                (1-contact_sequence_dm30(1,conp1)*contact_sequence_dm30(1,conp1_next))*(x0+front_off+3/2*Tstance*localvelocity);
        
        x_des((i+1)*NFS+14) = contact_sequence_dm30(2,conp1_next)*contact_sequence_dm30(2,conp1)*x_des(i*NFS+14)+
                                (1-contact_sequence_dm30(2,conp1)*contact_sequence_dm30(2,conp1_next))*(x0+rear_off+3/2*Tstance*localvelocity);
        
        x_des((i+1)*NFS+15) = contact_sequence_dm30(3,conp1_next)*contact_sequence_dm30(3,conp1)*x_des(i*NFS+15)+
        ...
        x_des((HORIZ+1)*NFS+i*NFI+15) = (1-contact_sequence_dm30(3,conp1))*vRaibstep;//0.4

        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*i+4) = contact_sequence_dm30(0,conp1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*i+5) = contact_sequence_dm30(1,conp1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*i+6) = contact_sequence_dm30(2,conp1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*i+7) = contact_sequence_dm30(3,conp1);
    }

    conp1 = (controlTick+HORIZ)%60;

    x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*(HORIZ+1)) = contact_sequence_dm30(0,conp1);
    x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*(HORIZ+1)+1) = contact_sequence_dm30(1,conp1);
    x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*(HORIZ+1)+2) = contact_sequence_dm30(2,conp1);
    x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*(HORIZ+1)+3) = contact_sequence_dm30(3,conp1);
...
casadi::DM SRBNMPC::motionPlannerN_MT15(casadi::DM q0, size_t controlTick){
    ...
    //     x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+3) = 1;//contact_sequence_dm(3,conp1);
    // }else{
        conm1 = (controlTick-1)%30;
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)) = contact_sequence_dm15(0,conm1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+1) = contact_sequence_dm15(1,conm1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+2) = contact_sequence_dm15(2,conm1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+3) = contact_sequence_dm15(3,conm1);
    // }

    if(starttrotting){
        if(controlTick%300 == 0){
    ...
    for(size_t i= 0; i< HORIZ; i++){
        ...
        //if(controlTick+i>0){
            conp1_next = (controlTick+i+1)%30;
        //}
        if((controlTick+i)%15 == 0){
            x0 = x_des(i*NFS);
        }
        ...
        x_des((HORIZ+1)*NFS+i*NFI+8) = contact_sequence_dm15(2,conp1)*MASS*9.81/(contact_sequence_dm15(2,conp1)+contact_sequence_dm15(3,conp1));
        x_des((HORIZ+1)*NFS+i*NFI+11) = contact_sequence_dm15(3,conp1)*MASS*9.81/(contact_sequence_dm15(2,conp1)+contact_sequence_dm15(3,conp1));
        
        x_des((HORIZ+1)*NFS+i*NFI+12) = (1-contact_sequence_dm15(0,conp1))*vRaibstep;//0.4
        x_des((HORIZ+1)*NFS+i*NFI+13) = (1-contact_sequence_dm15(1,conp1))*vRaibstep;//0.4
        x_des((HORIZ+1)*NFS+i*NFI+14) = (1-contact_sequence_dm15(2,conp1))*vRaibstep;//0.4
        x_des((HORIZ+1)*NFS+i*NFI+15) = (1-contact_sequence_dm15(3,conp1))*vRaibstep;//0.4

        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*i+4) = contact_sequence_dm15(0,conp1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*i+5) = contact_sequence_dm15(1,conp1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*i+6) = contact_sequence_dm15(2,conp1);
        x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*i+7) = contact_sequence_dm15(3,conp1);
    }

    conp1 = (controlTick+HORIZ)%30;

    x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*(HORIZ+1)) = contact_sequence_dm15(0,conp1);
    x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*(HORIZ+1)+1) = contact_sequence_dm15(1,conp1);
    x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*(HORIZ+1)+2) = contact_sequence_dm15(2,conp1);
    x_des(NFS*(HORIZ+1)+NFI*(HORIZ)+4*(HORIZ+1)+3) = contact_sequence_dm15(3,conp1);
...
int* SRBNMPC::returnConInd15(size_t controlTick){
    int controlMPC = std::floor((controlTick-21989)/20);
    static int conInd[5] = {0,0,0,0,0};
    
    int conmark = controlMPC%30;
    conInd[0] = static_cast<double>(contact_sequence_dm15(0,conmark));
    conInd[1] = static_cast<double>(contact_sequence_dm15(1,conmark));
    conInd[2] = static_cast<double>(contact_sequence_dm15(2,conmark));

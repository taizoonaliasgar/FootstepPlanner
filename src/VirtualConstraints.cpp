        ...
        for(size_t i=0; i<4; i++){
            if(con->ind[i]==0){
                ...
                if(i>1){
                    ...
                    double az[8]= {traj->toeInit(2,i), traj->toeInit(2,i), 2*params->swingHeight, 3*params->swingHeight, 
                                3*params->swingHeight, 2*params->swingHeight, 0.029,0.019};//traj->toeInit(2,i),traj->toeInit(2,i)};//0.02, 0.02};//to+0.01, to};
                    calcBezierAll(n, az, phase, tz);
                    ...
                    double ay[8]= {traj->toeInit(1,i), traj->toeInit(1,i), pow(-1,i+1)*wall_y+pow(-1,i)*params->swingHeight, pow(-1,i+1)*wall_y+pow(-1,i)*params->swingHeight*2, 
                                pow(-1,i+1)*wall_y+pow(-1,i)*params->swingHeight*2, pow(-1,i+1)*wall_y+pow(-1,i)*params->swingHeight, pow(-1,i+1)*wall_y, pow(-1,i+1)*wall_y};
...
void VirtCon::updateVirtualConstraintswalkR(const StateInfo *state, const KinInf *kin, const TrajInfo *traj, const ConInf *con, size_t gait, double frontphase, double rearphase, MP *params, const LLInfo *ll){
    size_t outDim = 6+3*(4-con->cnt);
    size_t conDim = 3*con->cnt;
